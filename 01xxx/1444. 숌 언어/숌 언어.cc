...
int getIdx(char a, char b) {
    if(a>='a') return (a-'a')*52 + (b-'A'+26);
    return (a-'A'+26)*52 + (b-'a');
}
...
int main() {
    ...
    int st=getIdx(s[0], s[1]), en=getIdx(s[n-2], s[n-1]);
    HopcroftKarp hk(52*52, 52*52);
    for(int i=1;i<n-1;i++) {
        int a = getIdx(s[i-1], s[i]);
        int b = getIdx(s[i], s[i+1]);
        if(a==st || a==en || b==st || b==en) continue;
        if(s[i]>='a') hk.edge(a, b);
        else hk.edge(b, a);
    }
    cout << hk.res() + (st==en ? 1 : 2);
}