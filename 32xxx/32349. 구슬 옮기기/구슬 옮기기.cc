...
vector<pii> ball1, ball2;
bool board1[100][100], board2[100][100];

int main() {
    cin.tie(0)->sync_with_stdio(0);
    int n, m; cin >> n >> m;
    for(int i=0;i<m;i++) for(int j=0;j<n;j++) cin >> board1[i][j];
    for(int i=0;i<m;i++) for(int j=0;j<n;j++) cin >> board2[i][j];
    for(int i=0;i<m;i++) {
        for(int j=0;j<n;j++) {
            if(board1[i][j]==board2[i][j]) continue;
            if(board1[i][j]) ball1.push_back({i, j});
            else ball2.push_back({i, j});
        }
    }
    if(ball1.size()!=ball2.size()) return !(cout << -1);

    HopcroftKarp hk(ball1.size(), ball2.size());
    for(int i=0;i<ball1.size();i++) {
        for(int j=0;j<ball2.size();j++) {
            if(abs(ball2[i].ff-ball1[j].ff)+abs(ball2[i].ss-ball1[j].ss)==1) hk.edge(i, j);
        }
    }
    cout << 2*ball1.size()-hk.res();
}