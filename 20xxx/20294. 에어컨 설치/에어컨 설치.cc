...
int main() {
    ...
    int n; cin >> n;
    for(int i=0;i<n;i++) cin >> pos[i][0] >> pos[i][1] >> pos[i][2];
    ...
    for(int i=0;i<n;i++) {
        bool conned=false;
        for(int j=0;j<n;j++) {
            int a = pos[i][0]+pos[i][1]+pos[i][2];
            int b = pos[j][0]+pos[j][1]+pos[j][2];
            if(abs(pos[i][0]-pos[j][0])+abs(pos[i][1]-pos[j][1])+abs(pos[i][2]-pos[j][2])==1) {
                conned=true;
                if(!(a%2) && b%2) {
                    hk.edge(i, j);
                }
