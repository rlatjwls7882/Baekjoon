...
int main() {
    ...
    vector<ConvexHull::point> v(n+3);
    for(int i=0;i<n+3;i++) cin >> v[i].x >> v[i].y;

    int l=3, r=n+2;
    while(l<r) {
        int m=l+r>>1;
        vector<ConvexHull::point> cur;
        for(int i=0;i<=m;i++) cur.push_back(v[i]);
    ...
    vector<ConvexHull::point> cur;
    for(int i=0;i<=l;i++) cur.push_back(v[i]);
    ConvexHull ch(cur);
    if(ch.area()<a*2) cout << "draw";
    else if((l-3)%2) cout << "wider";
    else cout << "wapas";
}