...
int main() {
    ...
    while(l<r) {
        ...
        for(int i=0;i<n;i++) {
            for(int j=0;j<m;j++) {
                if(arr[i][j]>mid) hk.edge(i, j);
            }
        }
        hk.res();
        auto cover = hk.cover();
        if(cover[0].size()+cover[1].size()>k) l=mid+1;
        else r=mid;
    }
    ...
    for(int i=0;i<n;i++) {
        for(int j=0;j<m;j++) {
            if(arr[i][j]>l) hk.edge(i, j);
        }
    }
    hk.res();
    auto cover = hk.cover();

    int rem = k-(cover[0].size()+cover[1].size());
    for(int i=0;i<n;i++) {
        bool chk=true;
        for(int e:cover[0]) {
            if(e==i) chk=false;
