...
int main() {
    ...
    while(t--) {
        ...
        for(int i=1;i<hull.size();i++) {
            if(hull[i].y>hull[idx].y || hull[i].y==hull[idx].y && hull[i].x<hull[idx].x) idx=i;
        }

        cout << hull.size() << '\n';
        for(int i=idx;i>=0;i--) cout << hull[i].x << ' ' << hull[i].y << '\n';
        for(int i=hull.size()-1;i>idx;i--) cout << hull[i].x << ' ' << hull[i].y << '\n';
    }
}