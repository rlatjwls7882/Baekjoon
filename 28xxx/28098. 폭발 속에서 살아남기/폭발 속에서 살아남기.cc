...
struct ConvexHull {
    ...
    inline ConvexHull(vector<point> &v) {
        ...
        int revIdx=hull.size()-1;
        while(revIdx>1 && ccw(hull[0], hull[revIdx-1], hull[revIdx])==0) revIdx--;
        reverse(hull.begin()+revIdx, hull.end());
    }
...
int main() {
    ...
    for(int i=0;i<hull.size();i++) {
        if(ch.ccw(hull[i], {0, 0}, hull[(i+1)%hull.size()])>=0) {
            cout << "Yes";
            return 0;
        }
    }
